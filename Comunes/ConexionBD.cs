...
namespace Examen1P_Chico.Comunes
{
    public class ConexionBD
    {
        ...
        public static List<HistoricoEquipos> GetHistoricoEquipos(string cedula)
        {
            DataSet ds = new DataSet();
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = abrirconexion();
        ...
        private static List<HistoricoEquipos> fillHistorico(DataSet ds)
        {
            List<HistoricoEquipos> historico = new List<HistoricoEquipos>();
            HistoricoEquipos objHistorico = new HistoricoEquipos();
            for (int i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
            {
                objHistorico = new HistoricoEquipos();
